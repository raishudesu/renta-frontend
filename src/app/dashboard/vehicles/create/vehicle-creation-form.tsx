...
export default function VehicleCreationForm({
  ...
  const form = useForm<z.infer<typeof vehicleCreationSchema>>({
  ...
  const { execute, isPending } = useAction(createVehicleAction, {
    onError: ({ error }) => {
  ...
  const onSubmit = async (values: z.infer<typeof vehicleCreationSchema>) => {
    try {
      const finalValues = {
        ...values,
